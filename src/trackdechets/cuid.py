"""
Copy/paste from https://github.com/necaris/cuid.py/blob/master/test_cuid.py with a stronger random generator
...
def _pad(string, size):  # type: (str, int) -> str
    ...
    if strlen == size:
        return string
    if strlen < size:
        return _PADDING[0 : size - strlen] + string  # noqa
    return string[-size:]
...
def _random_block():  # type: () -> str
    ...
    :rtype: str
    """
    random_number = secrets.choice(range(0, DISCRETE_VALUES))
